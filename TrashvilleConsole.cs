    ...
    internal static class TrashvilleConsole
    {
        ...
        private static bool Dispatch(string[] p)
        {
            ...
            try
            {
                switch (cmd)
                {
                    ...
                    case "realview": case "realradius": if (p.Length > 2 && float.TryParse(p[2], NumberStyles.Float, CultureInfo.InvariantCulture, out float rr)) Instanced.Virtualizer.ViewDist = Mathf.Clamp(rr, 4f, 60f); Log($"realView (MAX interactable radius) = {Instanced.Virtualizer.ViewDist}; current adaptive radius = {Instanced.Virtualizer.MatRadius:F1}"); break;
        ...
        private static void Status()
        {
            ...
            Log($"armed={Preferences.ArmBenchmark} bypass={Preferences.BypassCap} opt={Preferences.OptimizeClones} mode={(Preferences.SpawnKinematic ? "kinematic" : "dynamic")} | " +
                $"clones={CloneRegistry.Count} awake={CloneRegistry.CountAwake()} instanced={Instanced.InstancedTrash.Count} types={Instanced.InstancedTrash.TypeCount} real={Instanced.Virtualizer.RealCount} matR={Instanced.Virtualizer.MatRadius:F1} viewMax={Instanced.Virtualizer.ViewDist:F0} virt={Instanced.Virtualizer.Enabled} budget={Preferences.MaxAwakeBudget} gameTrash={TrashRegistry.Count} mgr={mgr} pending={TrashSpawner.Pending} | " +
