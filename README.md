...
A performance mod for **Schedule I** (MelonLoader / IL2CPP) that lets the world hold far
more trash with little cost, **without losing any base-game behaviour**.
...
Trashville routes the game's generated trash into a cheap **GPU-instanced field** (rendered
...
3. Drop `Trashville.dll` into the game's `Mods/` folder.
...
`Trashville_01_Main`) or the Mod Manager phone app:
...
| `MaxRealItems` | 600 | How many trash items are real/interactable around you at once (the perf vs interaction-range dial). |
| `MaterializeDistance` | 32 | How far ahead (metres) trash becomes interactable. |
| `TrashMultiplier` | 1 | Multiply the game's own trash output. 1 = vanilla amount; higher = more litter, kept cheap by the layer. |
...
host-authoritative multiplayer support is a planned follow-up (`TODO` in the source).
...
- `dotnet build -c Debug` - adds the benchmark/ablation harness: an on-screen HUD, dev
  hotkeys (F5 arm, F7/F8/F9 spawn 100/1k/10k, F11 sweep, F6 HUD), and a `tv ...` dev
  console. All of it is compiled out of Release via `#if DEBUG`.
